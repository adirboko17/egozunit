(function () {
  ...
  function renderEvents(events) {
    ...
    if (!upcoming.length) {
      container.innerHTML =
        '<div class="account-empty">' +
          ...
          '<a href="/events" class="btn btn--ghost" style="margin-top:18px;">' + esc(t('events.empty.btn', 'לכל האירועים')) + '</a>' +
        '</div>';
      return;
    }
    ...
    container.innerHTML = upcoming.slice(0, 8).map(function (event) {
      ...
      var url = event.slug ? ('/event?slug=' + encodeURIComponent(event.slug)) : (event.cta_url || '/events');
      var meta = [];
  ...
  async function boot() {
    ...
    if (!profile || !profile.registration_completed) {
      window.location.replace('/join?step=2');
      return;
    }
